...
async def make_request(client, output_tokens, request_timeout, use_long_context):
    ...
    try:
        stream = await client.chat.completions.create(
            model="llm/qwen/Qwen2-0.5B-Instruct",
            messages=[
...
async def run_benchmark(num_requests, concurrency, request_timeout, output_tokens, vllm_url, api_key, use_long_context):
    ...
    token_list = [tokens for tokens, _, _, _ in results if tokens is not None]
    ...
    tpot_list = [x / y for x, y in zip(latencies, token_list)]
    ...
    tpot_percentiles = [calculate_percentile(avg_tpot, p) for p in percentiles]
 ...
 # 注意，由于qa的分布不一样，latency肯定是不同的，tpot=latency/tokens不同，ttft应该差不多，因为不取决于token个数
...
# 开启服务 vllm serve llm/qwen/Qwen2-0.5B-Instruct --tensor-parallel-size 3
# 开启压测 python stress/vllm_benchmark.py --num_requests 200 --concurrency 30 --output_tokens 200 --vllm_url "http://localhost:8000/v1" --api_key "EMPTY"
...
#   "total_time": 22.90266466140747,
#   "requests_per_second": 8.73260832120611,
#   "total_output_tokens": 37271,
#   "latency": {
#     "average": 3.29976065158844,
#     "p50": 4.003111243247986,
#     "p95": 4.285609352588653,
#     "p99": 4.295872704982758
#   },
#   "tokens_per_second": {
#     "average": 62.04054955423974,
#     "p50": 48.855524492160846,
#     "p95": 46.59948335593686,
#     "p99": 45.256155325481785
#   },
#   "time_to_first_token(ttft)": {
#     "average": 0.09516718626022339,
#     "p50": 0.10111618041992188,
#     "p95": 0.15029379129409787,
#     "p99": 0.15760119915008539
#   },
#   "time-per-output-token(tpot)": {
#     "average": 0.017659583500219275,
#     "p50": 0.017659583500219275,
#     "p95": 0.017659583500219275,
#     "p99": 0.017659583500219275
#   }
# }
